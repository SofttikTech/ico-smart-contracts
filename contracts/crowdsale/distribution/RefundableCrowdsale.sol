...
contract RefundableCrowdsale is Context, FinalizableCrowdsale {
    ...
    function claimRefund(address payable refundee) public {
        ...
        _escrow.withdrawWithGas(refundee, USDT);
    }
    ...
    function _finalization() internal {
        if (goalReached()) {
            _escrow.close();
            _escrow.beneficiaryWithdraw(USDT);
        } else {
    ...
    function _forwardFunds(address beneficiary, uint256 weiAmount) internal {
        _escrow.deposit(weiAmount, _msgSender());
        USDT.transferFrom(beneficiary, address(_escrow), weiAmount);
    }
}
